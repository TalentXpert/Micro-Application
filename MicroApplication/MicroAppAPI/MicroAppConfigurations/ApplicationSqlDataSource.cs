...
{
    public class ApplicationSqlDataSource : BaseSqlDataSource
    {
        public static string Chart = "Chart";

        protected override List<SqlDataSource> GetApplicationSqlDataSources()
        {
            return new List<SqlDataSource>
            {
               new SqlDataSource("E95B2FAC-29C0-C620-80BB-08DC01368872","Noise Pollution","select SensorId TX_Sensor,Avg(NoisePollution) NU_AvgNoise,Max(NoisePollution) MaxNoise,Min(NoisePollution) NU_MinNoise  from [PollutionData] group by SensorId",SqlDataSourceType.Chart),
               new SqlDataSource("5C665428-3401-4319-8C2C-2D9771D59E6B","Average Pollution","select SensorId TX_Sensor,Avg(NoisePollution) NU_Noise from [PollutionData] group by SensorId",SqlDataSourceType.Chart)
            };
        }
    }
