...
{
    public class ApplicationDashboard: BaseDashboard
    {
        public static Guid MainDashboardId = Guid.Parse("C0F55AAC-F354-4E53-AD2F-2CA1D9A05CF0");
        public static List<DashboardSchema> GetDashbaords()
        {
            var db = new DashboardSchema
            {
                Id = MainDashboardId,
                Name = "Noise Pollution",
                MinimumRowHeight = 400,
                MenuId = ApplicationMenu.DashboardId,
                Position = 1
            };
            ...
            var panel = new DashboardPanel(DashboardPanelContentType.Graph, ApplicationChart.MaxNoisePollutionId, "Locality Maximum Noise Pollution", 1, 400);
            row.AddPanel(panel);

            panel = new DashboardPanel(DashboardPanelContentType.Graph, ApplicationChart.MinNoisePollutionId, "Locality Minimum Noise Pollution", 2, 400);
            row.AddPanel(panel);
            db.Rows.Add(row);
            ...
            panel = new DashboardPanel(DashboardPanelContentType.Graph, ApplicationChart.AvgNoisePollutionId, "Locality Average Noise Pollution", 1, 400);
            row.AddPanel(panel);

            panel = new DashboardPanel(DashboardPanelContentType.Graph, ApplicationChart.NoisePollutionId, "Locality Noise Pollution", 2, 400);
            row.AddPanel(panel);
            db.Rows.Add(row);
           
            return new List<DashboardSchema> { db };

        }

        protected override List<DashboardSchema> GetApplicationDashboards()
        {
            return GetDashbaords();
        }
    }
